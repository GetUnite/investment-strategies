...
describe("VoteExecutor", function () {
  ...
  beforeEach(async () => {
    ...
    const Executor = await ethers.getContractFactory("VoteExecutorForTest") as VoteExecutor__factory;
    executor = await Executor.deploy(signers[0].address, strategy.address, investor.address,[]);
