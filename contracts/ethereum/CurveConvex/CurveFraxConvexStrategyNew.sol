...
contract CurveFraxConvexStrategyNew is AccessControl, IAlluoStrategy {
        ...
        returns (bytes memory)
    {
        (
            ...
            IERC20 crvLpToken,
            IERC20 poolToken,
            uint8 poolSize, // not used
            uint8 tokenIndexInCurve,
            uint256 poolId, // not used
            IERC20 stakeToken,
            address fraxPool,
            uint256 duration
        ...
        uint256[2] memory twoPoolTokensAmount;
        twoPoolTokensAmount[tokenIndexInCurve] = amount;
        ...
        curveCall = abi.encodeWithSelector(
            0x0b4c7e4d,
            uint256[2]([twoPoolTokensAmount[0], twoPoolTokensAmount[1]]),
            0
        );
    ...
    function exitAll(
        ...
        (
            ...
            IERC20 crvLpToken,
            uint8 tokenIndexInCurve,
            uint256 convexPoolId, // not used
            address fraxPool,
            bytes32 kek_id // not used
        ) = decodeExitParams(data);
        uint256 lpAmount;

        kek_id = kek_ids[fraxPool];
        ...
        // manage exchange the rewards and send to the reciever
